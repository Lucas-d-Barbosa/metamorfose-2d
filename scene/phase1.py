...
class Phase1Scene(BaseScene):
    ...
    def _phase_end_update(self, dt: float) -> None:
        ...
        if self._phase_end_timer >= 9.0 and not self._phase_end_done:
            ...
            self.flags.phase = 2
            save_mgr.save(self.flags, self.player)
            self.transition_to("game_over")  # placeholder until Phase 2
    ...
    def _resolve_collisions(self) -> None:
        for wall in self.tilemap.wall_rects:
            if not self.player.hitbox.colliderect(wall):
                continue
            ol = self.player.hitbox.right - wall.left
            or_ = wall.right - self.player.hitbox.left
            ot = self.player.hitbox.bottom - wall.top
            ob = wall.bottom - self.player.hitbox.top
            m = min(ol, or_, ot, ob)
            if m == ol:
                self.player.move_x(-ol);   self.player.velocity.x = 0
            elif m == or_:
                self.player.move_x(or_);   self.player.velocity.x = 0
            elif m == ot:
                self.player.move_y(-ot);   self.player.velocity.y = 0
            else:
                self.player.move_y(ob);    self.player.velocity.y = 0
